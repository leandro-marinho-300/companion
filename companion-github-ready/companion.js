...
function render(){
  const checkpointReady = state.activation.status==='running' && (state.activation.checkpointReady || (state.activation.checkpointDue && Date.now() >= new Date(state.activation.checkpointDue).getTime()));
  const f=focus();
  ...
  else if(state.activation.status==='running'){label='ATIVAÇÃO';color='var(--yellow)';width='54%'}
  ...
  $('#comp-normal-actions').hidden = checkpointReady;
  $('#comp-checkpoint').hidden = !checkpointReady;
}
...
function markEngagedFromCompanion(){
  ...
  state.activation.status='engaged'; state.activation.checkpointReady=false; state.activation.checkpointDue=null;
  persist();
}
...
window.addEventListener('storage',()=>{state=S.loadState();render()});window.addEventListener('keydown',e=>{if(e.key==='Escape')closeOverlay();if(e.ctrlKey&&e.shiftKey&&e.code==='Space'){e.preventDefault();showInterruption()}});
render();
